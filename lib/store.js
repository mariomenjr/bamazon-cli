...
Store.prototype.updateInventory = function(answers) {
  conn.query("UPDATE bm_products SET stock_qty = (stock_qty - ?) WHERE id = ?", [Number(answers.sale_qty), Number(answers.id)], (err, results, fields) => {
...
Store.prototype.showAll = function() {
  conn.query("SELECT bp.*, bd.dept_name FROM bm_products bp LEFT JOIN bm_depts bd ON bp.dept_fk = bd.id", (err, results, fields) => {
    this.data = results;
